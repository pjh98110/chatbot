...
def on_input_change():
    ...
    if st.session_state['chat_stage'] == 1:
        ...
        try:
            ...
            target_str = f"""데이터 분석가로서 답변해줘. {data.iloc[idx]["월별"]} {input_str}의 
                총인구는 {data.iloc[idx]["총인구"]}명으로 예측되며, 
                {input_str}의 인구 증가 요인은 다음과 같다.
                수도권 외부 유입: 도심규제완화, 도시 접근성: GTX-A와 신안산선, 
                산업단지 개발: 삼성전자 반도체 공장, 아동친화도시: 아동친화도시 10가지 원칙,
                임신 출산 복지사업: 22년 출생 수 전국 2위 달성, 큰 행정면적:(서울시의 1.4배, 경기도 2위)이다.
                기대효과는 다음과 같다.
                자원 및 예산 배분, 도시 계획, 교통 계획, 교육 서비스 개선, 지역 균형발전 정책 수립, 사회 복지프로그램 계획이다.
                앞으로 {input_str}의 미래 총인구가 증가하는 이유, 변동성, 기대효과에 대해 보고서 형태로 자세하게 설명해줘"""
